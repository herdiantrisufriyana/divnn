...
```
```{r include=FALSE}
tidy_set=read_ts_tar_gz('quick-start-R/tidy_set_R.ts.tar.gz')
```
...
write_ts_tar_gz(tidy_set,'quick-start-R/tidy_set_R')
```
...
ontonet=
  tidy_set %>%
  ontonet_generator(path='quick-start-R/ontonet_R')
```
```{r include=FALSE}
ontonet=
