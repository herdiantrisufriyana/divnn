...
```{r Load this and other package, include=FALSE}
devtools::load_all(".")
library(divnn)
```
...
```
```{r include=FALSE}
tidy_set=TidySet.read(path='quick-start/tidy_set_py.ts.tar.gz')
```
...
TidySet.write(tidy_set,'quick-start/tidy_set_R')
```
...
tidy_set=TidySet.read('quick-start/tidy_set_R.ts.tar.gz')
```
...
ontonet=
  tidy_set %>%
  generator.ontonet(path='quick-start/ontonet_R')
```
```{r include=FALSE}
ontonet=
