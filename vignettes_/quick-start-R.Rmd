---
title: "DeepInsight Visible Neural Network: Quick Start divnn R"
author:
...
You can install this package using function install_github() from devtools
package. Then load this package.

```{r Showing install and load this package, eval=FALSE, include=TRUE}
devtools::install_github('herdiantrisufriyana/divnn')
library(divnn)
```

Several packages are needed to load explicitly since training and the other
functions are not included in this package. For simplicity, code to turn off
your GPU is also provided below.

```{r Showing load packages, eval=FALSE, include=TRUE}
library(tidyverse)
...
input$value %>%
  ...
  group_by(outcome) %>%
  summarize_all(function(x)paste(round(mean(x),2),'±',round(sd(x),2))) %>%
...
input$mapping %>%
  ...
  knitr::kable(
    format='html'
    ,caption='Feature three-dimensional mapping matrix'
  ) %>%
  kableExtra::kable_styling(full_width=T)
```
...
```
```{r include=FALSE}
tidy_set=TidySet.read('quick-start-R/tidy_set_R.ts.tar.gz')
```
...
TidySet.write(tidy_set,'quick-start-R/tidy_set_R')
```
...
ontonet=
  tidy_set %>%
  generator.ontonet(path='quick-start-R/ontonet_R')
```
```{r include=FALSE}
ontonet=
  readLines('quick-start-R/ontonet_R.json') %>%
  model_from_json()
```
...
The LR will be reduced by factor of 0.1 at iteration 30, 60, and 80. Early
stopping will also happen if validation loss does not decrease >0.001 after
30 iterations. The best weight at minimum validation loss will be applied.
...
```
```{r include=FALSE}
history=readRDS('quick-start-R/history_R.rds')
load_model_weights_hdf5(ontonet,'quick-start-R/ontonet_R.h5')
```
...
```
```{r include=FALSE}
evaluation=readRDS('quick-start-R/evaluation_R.rds')
```
